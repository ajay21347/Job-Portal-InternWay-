...
def api_signup():
    ...
    if database.add_user(username, password, email, user_type):
        ...
        return jsonify({'success': False, 'message': 'Signup failed. Username or email might be taken.'})
...
def api_post_job():
    ...
    if database.add_job(recruiter_id, title, company, location, description, salary, job_type, job_level, employment_type):
...
def api_apply_job():
    ...
    if database.apply_for_job(job_id, seeker_id):
        ...
        return jsonify({'success': False, 'message': 'Failed to submit application or already applied.'})
...
def api_get_seeker_applications():
    if 'user_id' not in session or session['user_type'] != 'seeker':
        return jsonify({'success': False, 'message': 'Unauthorized', 'applications': []}), 401
