document.addEventListener("DOMContentLoaded", () => {
  ...
  const recruiterProfileImage = document.getElementById(
  ...
  const searchEmploymentType = document.getElementById(
  ...
  function updateHeaderAndDisplaySections() {
    ...
    if (currentUser && currentUser.user_id) {
      ...
      welcomeSpan.textContent = `Welcome, ${currentUser.username} (${
        currentUser.user_type === "recruiter" ? "Recruiter" : "Seeker"
      })`;
      ...
      if (currentUser.user_type === "recruiter") {
        recruiterProfileImage.src =
          currentUser.profile_image_url ||
          "https://placehold.co/80x80/4A90E2/FFFFFF?text=R";
        ...
        seekerProfileImage.src =
          currentUser.profile_image_url ||
          "https://placehold.co/80x80/7D4AE2/FFFFFF?text=S";
  ...
  if (postJobForm) {
    postJobForm.addEventListener("submit", async (e) => {
      ...
      const jobLevel = formData.get("job_level");
      if (jobLevel) {
        formData.set("job_level", jobLevel.toLowerCase());
      }

      const employmentType = formData.get("employment_type");
      if (employmentType) {
        formData.set("employment_type", employmentType.toLowerCase());
      }
  ...
  async function fetchPostedJobs() {
    ...
    try {
      ...
      if (data.success && data.jobs) {
        ...
        jobListContainer.innerHTML = `<p class="error-message">${
          data.message || "Error loading jobs."
        }</p>`;
      }
    } catch (error) {
      console.error("Error fetching posted jobs:", error);
      jobListContainer.innerHTML =
        '<p class="error-message">Could not load jobs.</p>';
    }
  }
  ...
  async function fetchAvailableJobs(filters = {}) {
    ...
    try {
      ...
      if (data.success && data.jobs) {
        ...
        if (data.jobs.length === 0) {
          availableJobsContainer.innerHTML =
            "<p>No jobs available at the moment matching your criteria.</p>";
        } else {
          data.jobs.forEach((job) => {
            const jobDiv = document.createElement("div");
            jobDiv.classList.add("job-item");
        ...
        availableJobsContainer.innerHTML = `<p class="error-message">${
          data.message || "Error loading available jobs."
        }</p>`;
      }
    } catch (error) {
      console.error("Error fetching available jobs:", error);
      availableJobsContainer.innerHTML =
        '<p class="error-message">Could not load available jobs.</p>';
    }
  }
  ...
  async function fetchAppliedJobs() {
    ...
    try {
      ...
      if (data.success && data.applications) {
        ...
        appliedJobsContainer.innerHTML = `<p class="error-message">${
          data.message || "Error loading applied jobs."
        }</p>`;
      }
    } catch (error) {
      console.error("Error fetching applied jobs:", error);
      appliedJobsContainer.innerHTML =
        '<p class="error-message">Could not load applied jobs.</p>';
    }
  }
